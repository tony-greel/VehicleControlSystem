package com.example.lijunjie.vehiclecontrolsystem.base.view;
...
public class DiscView extends View {
    ...
    public DiscView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        textPaint.setColor(Color.parseColor("#0f81f4"));
    ...
    private void drawDataLine(Point start, Point end, Canvas canvas, Databean item){
        Log.e("AAA",start+"  "+end);
        mPaint.setColor(item.getColor());
    ...
    private void drawText(Canvas canvas, Point point, Databean item) {
    ...
    public void setItems(List<Databean> items) {
        this.items = items;
        total = 0;
        for (Databean item : items) {
            total += item.getValue();
